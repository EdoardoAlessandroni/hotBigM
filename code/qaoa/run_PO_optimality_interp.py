"""Train ma-QAOA on PO / optimality with E_f = P75 AND INTERP warm starts (1 -> 2 -> ... -> p).

Identical to run_PO_optimality_percentile.py -- same grids, same qaoa_samples/qaoa_steps, same
pickled payload and the same filenames -- except warm_start=True and the output folder. Depth p
is initialized from the depth-(p-1) optimum instead of a random init, which removes the p=3 0/1
bimodality (n=12 pilot: 4/10 collapses -> 0/10, mean eta 0.59 -> 0.95, sd 0.46 -> 0.10).

NOTE p=3 now trains three circuits per cell, so expect ~2-3x the cold-start runtime (~100 min).
vseed is the outermost loop, so a complete (all sizes x all layers) slice is on disk after each
instance.
"""
...
Ef_percentile = 75
outdir = f"./data_qaoa/{problem_type}_{M_strategy}_maqaoa_percentile_interp"
        ...
        for j, layers in enumerate(layers_list):
            for k, eta_req in enumerate(eta_req_list):
                ...
                res = {
                    ...
                    "warm_start":         True,   # extra key; the folders are otherwise identical
                }
