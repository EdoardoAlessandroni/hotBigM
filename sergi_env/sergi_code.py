import numpy as np
import random
from myalgo import M_method_feas, M_method_opt
import pickle
import sys
from timeit import default_timer as timer
from qiskit_optimization.translators import from_docplex_mp
from docplex.mp.model_reader import ModelReader
import cvxpy as cp



##### Useful functions

### general

def symmetrize(Q):
    return (Q + Q.T) / 2

def L1_norm(Q, const):
    return const + np.sum(np.abs(Q))

def L1_norm_hot(Q, const, n_bits, temperature, min_pfeas):
    return L1_norm(Q, const) + temperature * (n_bits * np.log(2) - np.log(1-min_pfeas))

def evaluate_energy(solution, Q, const):
    return const + np.dot(solution, np.dot(Q, solution))

def select_Ef(problem_type, n_bits):
    """ Choose E_f prop. to the square of nbits, since the number of terms in the Hamiltonian increase like this. 
    The costant is chosen such that it's possible to sample even at eta_req = 0.75 at temperatures = DA_temp * 100 """
    if problem_type == "NPP":
        return 3e3 * n_bits**2
    elif problem_type == "TSP_circle":
        return 3e5 * n_bits**2
    elif problem_type == "TSP_rand":
        return 3e5 * n_bits**2
    elif problem_type == "PO":
        return 3e-2 * n_bits**2

# NPP specifics

def get_QUBO_NPP(N, P, M, instance_seed, penalization=True, objective=True):
    """build a NPP QUBO with N integers, P partitions, M penalty factor, for problem identified with seed"""
    Q, const = np.zeros((N * P, N * P)), 0
    if objective:
        numbs = build_numbs_set(N, P, instance_seed)
        Ho, const_o = build_obj_NPP(numbs, N, P)
        Q += Ho
        const += const_o
    if penalization:
        Hp, const_p = build_pen_NPP(N, P)
        Q += M * Hp
        const += M * const_p
    return Q, const

def build_numbs_set(N, P, instance_seed):
    random.seed(instance_seed)
    numbs = np.array([int(1000 * random.uniform(0.0, 1.0)) for _ in range(N)])
    total = sum(numbs)
    if numbs[-1] - (total % P) > 1:
        numbs[-1] -= total % P
    else:
        numbs[-1] += P - (total % P)
    total = sum(numbs)
    return numbs

def idx_onehot(i, p, P):
    return i * P + p

def build_obj_NPP(numbs, N, P):
    H = np.zeros((N * P, N * P))
    alpha = np.sum(numbs) / P
    for p in range(P):
        for i in range(N):
            H[idx_onehot(i, p, P), idx_onehot(i, p, P)] = numbs[i]**2 - 2*alpha*numbs[i]
            for i_prime in range(N):
                if i_prime == i:
                    continue
                H[idx_onehot(i, p, P), idx_onehot(i_prime, p, P)] = numbs[i] * numbs[i_prime]
    const = P * alpha**2
    H = symmetrize(H)
    return H, const

def build_pen_NPP(N, P):
    H = np.zeros((N * P, N * P))
    for p in range(P):
        for i in range(N):
            H[idx_onehot(i, p, P), idx_onehot(i, p, P)] = -1
            for p_prime in range(P):
                if p_prime == p:
                    continue
                H[idx_onehot(i, p, P), idx_onehot(i, p_prime, P)] = 1
    const = N
    H = symmetrize(H)
    return H, const


# TSP specific

def build_adjacency(Nc, info_instance, info_type):
    if info_type == "circle":
        # trivial construction of adj. matrix of graph with N cities on a circle. Modify this function to get more complex structures
        coordinates = [(1_000_000 * np.cos((index / Nc) * 2 * np.pi), 1_000_000 * np.sin((index / Nc) * 2 * np.pi)) for index in range(Nc)]
        distance_table = [[int(np.sqrt((coordinates[c_1][0] - coordinates[c_0][0]) ** 2 + (coordinates[c_1][1] - coordinates[c_0][1]) ** 2)) for c_0 in range(Nc)] for c_1 in range(Nc)]
    elif info_type == "seed":
        # random construction of the adjacency matrix, based on euclidian distance. (cities are uniformly randomly placed inside a square)
        seed = info_instance
        np.random.seed(seed)
        coordinates = 1e6 * np.array( [np.random.uniform(low = -1, high = 1, size = 2) for i in range(Nc)] )
        distance_table = [[int(np.sqrt((coordinates[c_1][0] - coordinates[c_0][0]) ** 2 + (coordinates[c_1][1] - coordinates[c_0][1]) ** 2)) for c_0 in range(Nc)] for c_1 in range(Nc)]
    # elif info_type == "adjacency":
    #     distance_table = info_instance
    return np.array(distance_table)

def build_obj_TSP(distances, Nc):
    H = np.zeros((Nc**2, Nc**2))
    for t in range(Nc):
        for i in range(Nc):
            for i_prime in range(Nc):
                if i_prime != i:
                    H[idx_onehot(t, i, Nc), idx_onehot((t+1)%Nc, i_prime, Nc)] = distances[i, i_prime]
    H = symmetrize(H)
    const = 0
    return H, const

def build_pen_TSP(Nc):
    H = np.zeros((Nc**2, Nc**2))
    for t in range(Nc):
        for i in range(Nc):
            H[idx_onehot(t, i, Nc), idx_onehot(t, i, Nc)] = -2
            for i_prime in range(Nc):
                if i_prime != i:
                    H[idx_onehot(t, i, Nc), idx_onehot(t, i_prime, Nc)] += 1
            for t_prime in range(Nc):
                if t_prime != t:
                    H[idx_onehot(t, i, Nc), idx_onehot(t_prime, i, Nc)] += 1
    const = 2*Nc
    H = symmetrize(H)
    return H, const


def get_QUBO_TSP(Nc, M, instance_seed, info_type, objective = True, penalization=True):
    # build a TSP QUBO with Nc cities, M penalty factor, for problem identified with seed or on a circle
    Q, const = np.zeros((Nc**2, Nc**2)), 0
    if objective:
        distances = build_adjacency(Nc, instance_seed, info_type)
        Ho, const_o = build_obj_TSP(distances, Nc)
        Q += Ho
        const += const_o
    if penalization:
        Hp, const_p = build_pen_TSP(Nc)
        Q += M*Hp
        const += M*const_p
    return Q, const


# PO specific

def lower_bound_obj_PO(N, w, instance_seed):
    n_bits = N*w
    filename = f"instances/PO/{n_bits}/random{instance_seed}_{n_bits}.lp"
    m = ModelReader.read(filename, ignore_names=True)
    qp = from_docplex_mp(m)
    n = qp.get_num_binary_vars()
    obj = qp.objective
    Q = obj.quadratic.to_array()
    L = obj.linear.to_array()

    Q_tilde = np.ndarray((n+1, n+1))
    Q_tilde[1:, 1:] = Q
    Q_tilde[0, 0] = 0
    Q_tilde[0, 1:] = .5*L.T
    Q_tilde[1:, 0] = .5*L
    X = cp.Variable((n+1, n+1), symmetric=True)
    constraints = [X >> 0]
    constraints += [X[i,i] == X[0,i] for i in range(1, n+1) ]
    constraints += [X[0,0] == 1]      ###### alternative way of imposing boundness, rather than  0 <= X_ij <= 1  for all i,j (2 previous lines)
    prob = cp.Problem(cp.Minimize(cp.trace(Q_tilde @ X)), constraints)
    prob.solve(solver = "MOSEK")
    return prob.value

def build_pen_PO(N, w):
    ''' return quadratic part and constant part of the penalization part of the hamiltonian '''
    # constraints in the formulation      Ax = b 
    A = np.array( [2**i for i in range(w-1, -1, -1)]*N )
    b = 2**w - 1
    # build [H, const] from [A, b]. easier formulation since b is scalar and A only 1 row, since there's only 1 constraint.
    H = np.outer(A, A)
    H -= 2*b*np.diag(A)
    const = b**2
    return H, const

def get_PO_obj(n_bits, instance_seed):
    filename = f"instances/PO/{n_bits}/random{instance_seed}_{n_bits}.lp"
    m = ModelReader.read(filename, ignore_names=True)
    qp = from_docplex_mp(m)
    obj = qp.objective
    Q = obj.quadratic.to_array()
    L = obj.linear.to_array()
    return Q, L

def get_QUBO_PO(N, w, M, instance_seed, info_type, objective = True, penalization=True):
    ''' build a PO QUBO with N stocks, w qubits per stock, M penalty factor, for problem identified with seed '''
    Q, const = np.zeros(((N*w), (N*w))), 0
    if objective:
        if info_type == "seed":
            Q_o, L_o = get_PO_obj(N*w, instance_seed)
            Q += Q_o + np.diag(L_o)
        else:
            raise ValueError(f"info_type = {info_type} not understood for PO instance")
    if penalization:
        Hp, const_p = build_pen_PO(N, w)
        Q += M*Hp
        const += M*const_p
    return Q, const








def getQUBO_forThermal(problem_type, size, instance_seed, temperature, Mstrategy, eta_required, print_time = True, compare_Ml1 = False):
    """ Run an instance, first using our M algo to compute M^* and then sampling with SA the resulting QUBO.
     Fixed are the instance (seed and size), the Mstrategy and relative probablity required (eta) and the temperature schedule for SA, which copies DA's, scaled by a factor """
    t1 = timer()
    
    ## Set some parameters
    match problem_type:
        case "NPP":
            N, P = size
            n_bits = N * P
            info_type = "seed"
        case "TSP_circle" | "TSP_rand":
            Nc = size
            n_bits = Nc**2
            if problem_type == "TSP_circle":
                info_type = "circle"
            else:
                info_type = "seed"
        case "PO":
            N, w = size
            n_bits = N * w
            info_type = "seed"

    ## Set lower bound and other algorithm's parameters
    beta = 1 / temperature
    peak_max = 4 # 4 for NPP and TSP, 16 for PO
    if problem_type == "PO":
        E_LB = lower_bound_obj_PO(N, w, instance_seed)
        peak_max = 16
    else:
        E_LB = 0

    ## Compute M*
    if Mstrategy == "optimality":
        E_f = select_Ef(problem_type, n_bits)
        M_star, eta_guaranteed = M_method_opt(size, problem_type[:3], instance_seed, info_type, beta, peak_max, eta_required, E_f, E_LB, log_stable = True)
    elif Mstrategy == "feasibility":
        M_star, eta_guaranteed = M_method_feas(size, problem_type[:3], instance_seed, info_type, beta, peak_max, eta_required, E_LB, log_stable = True)
    
    
    ## Build QUBO(M^*)
    match problem_type:
        case "NPP":
            Q, const = get_QUBO_NPP(N, P, M_star, instance_seed)
        case "TSP_circle" | "TSP_rand":
            Q, const = get_QUBO_TSP(Nc, M_star, instance_seed, info_type)
        case "PO":
            Q, const = get_QUBO_PO(N, w, M_star, instance_seed, info_type)

    ## Some diagnostics
    print(f"Recommended M for strategy: {Mstrategy} is \t {M_star}")
    # print(f"Etas: required = {np.round(eta_required, 3)} \t guaranteed = {np.round(eta_guaranteed, 3)}")
    if print_time:
        t2 = timer()
        print(f"One instance with sizes = {size} of {problem_type} took {t2 - t1:.4f}s\n")

    if not compare_Ml1:
        return (Q, const)
    
    else:
        ## Compute M_l1 and its QUBO(M_l1) reformulation
        match problem_type:
            case "NPP":
                Q_obj, const_obj = get_QUBO_NPP(N, P, 1, instance_seed, penalization=False, objective=True)
            case "TSP_circle" | "TSP_rand":
                Q_obj, const_obj = get_QUBO_TSP(Nc, 1, instance_seed, info_type, penalization=False, objective=True)
            case "PO":
                Q_obj, const_obj = get_QUBO_PO(N, w, 1, instance_seed, info_type, penalization=False, objective=True)
        M_L1 = L1_norm_hot(Q_obj, const_obj, n_bits, temperature, eta_required)
        match problem_type:
            case "NPP":
                Q_l1, const_l1 = get_QUBO_NPP(N, P, M_L1, instance_seed)
            case "TSP_circle" | "TSP_rand":
                Q_l1, const_l1 = get_QUBO_TSP(Nc, M_L1, instance_seed, info_type)
            case "PO":
                Q_l1, const_l1 = get_QUBO_PO(N, w, M_L1, instance_seed, info_type)
        return (Q, const), (Q_l1, const_l1)
    
    





######### MAIN

### Set framework of the dataset and parameters
problem_type = "NPP" # "NPP" or "TSP_circle" or "TSP_rand" or "PO"
instance_seed = 42 # for PO, from 42 with jumps of 100 [42, 142, 242, ...]. For TSP_rand and NPP, from 42 [42, 43, 44, ...].
M_strategy = "feasibility"
eta_req = .5
temperature = 100
compare_Ml1 = True

match problem_type:
    case "NPP":
        N = 4 # from 3 until 8
        P = 3
        size = (N,P)
    case "TSP_circle" | "TSP_rand":
        N = 3 # from 2 to 5
        size = (N)
    case "PO":
        N = 4 # from 3 until 8
        w = 3
        size = (N,w)

if not compare_Ml1:
    Q, const = getQUBO_forThermal(problem_type, size, instance_seed, temperature, M_strategy, eta_req, compare_Ml1 = compare_Ml1)
else:
    (Q, const), (Q_l1, const_l1) = getQUBO_forThermal(problem_type, size, instance_seed, temperature, M_strategy, eta_req, compare_Ml1 = compare_Ml1)

# filename = ...
# data = ...
# file = open(filename, "wb")
# pickle.dump(data, file)
# file.close()